...
CloudTracker::CloudTracker() : m_cloud_coverage_percentage(0.0), m_avg_wind_speed(0.0) {}

CloudTracker::~CloudTracker() {}

bool CloudTracker::loadImages(const std::string& path1, const std::string& path2) {
...
cv::Mat CloudTracker::preprocessAndMask(const cv::Mat& src_gray, cv::Mat& out_mask) {
    cv::Mat blurred;
    // Apply Gaussian Blur to reduce noise
    cv::GaussianBlur(src_gray, blurred, cv::Size(5, 5), 0);
    
    // Perform thresholding to identify cloud regions.
    // In infrared satellite imagery, both land and ocean surfaces are warm (dark/black),
    // while clouds are cold (gray to white). A threshold around 50 captures most cloud cover
...
void CloudTracker::extractOverlays() {
    ...
    int num_labels = cv::connectedComponentsWithStats(candidate, labels, stats, centroids);
    ...
    for (int i = 1; i < num_labels; i++) {
        ...
        // Borders are thin lines (width >> height or height >> width, with small area relative to bounding box)
...
void CloudTracker::computeOpticalFlow() {
    ...
    // Note: We use the raw grayscale (or blurred) images for flow, as thresholded masks 
    // might lack the texture needed for good optical flow tracking.
    
    cv::calcOpticalFlowFarneback(m_img1_gray, m_img2_gray, m_flow, 
                                 0.5, 3, 15, 3, 5, 1.2, 0);
}

void CloudTracker::estimateWindSpeed(double timeDifference) {
    // Avoid division by zero
    if (timeDifference <= 0) timeDifference = 1.0;
    ...
    for (int y = 0; y < m_flow.rows; y += step) {
        for (int x = 0; x < m_flow.cols; x += step) {
            ...
            if (m_cloud_mask1.at<uchar>(y, x) > 0) {
                ...
                cv::arrowedLine(m_flow_visualization, pt1, pt2, color, 1, cv::LINE_AA, 0, 0.3);
            }
        }
    }
}
...
void CloudTracker::process(double timeDifference) {
    if (m_img1_gray.empty() || m_img2_gray.empty()) return;
    ...
    m_cloud_coverage_percentage = (static_cast<double>(cloud_pixels) / total_pixels) * 100.0;

    // Create highlighted clouds image (will be colored by wind speed in showResults)
...
void CloudTracker::drawLegend(cv::Mat& image) {
    ...
    if (start_x < 0 || start_y < 0) return; // Image too small
    ...
    cv::resize(color_gradient, color_gradient, cv::Size(legend_width, legend_height));
    ...
    cv::putText(image, text_mid, cv::Point(start_x + legend_width/2 - 15, start_y + legend_height + 20), 
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
    cv::putText(image, text_max, cv::Point(start_x + legend_width - 35, start_y + legend_height + 20), 
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
    cv::putText(image, "Wind Speed", cv::Point(start_x + legend_width/2 - 40, start_y - 10), 
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
}

void CloudTracker::showResults() {
    if (m_img1_color.empty()) return;
    ...
    cv::Mat masked_speed_vis = cv::Mat::zeros(wind_speed_vis.size(), wind_speed_vis.type());
    ...
    // Overlay country names in Yellow on Highlighted Clouds for contrast over the heatmap
    ...
    std::cout << "Cloud Coverage: " << m_cloud_coverage_percentage << " %" << std::endl;
    std::cout << "Average Wind Speed: " << m_avg_wind_speed << " units/time" << std::endl;
    std::cout << "------------------" << std::endl;
